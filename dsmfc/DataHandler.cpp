...
int CDataHandler::onData(evwork::IConn* pConn, const char* pData, size_t uSize)
{
	...
	while (uSize > 0)
	{
		if (uSize < DS_HEADER_SIZE)
			break;

		uint32_t uPktLen = Request::peekLen(pData);

		if ( (uPktLen < DS_HEADER_SIZE) || (m_uPacketLimit != (uint32_t)-1 && uPktLen > m_uPacketLimit) )
		{
			std::string strPeerIp = "";
			uint16_t uPeerPort = 0;
			pConn->getPeerInfo(strPeerIp, uPeerPort);

			LOG(Warn, "[ds::CDataHandler::%s] from:%s:%u recv a invalid packet, pktlen:%u", __FUNCTION__, strPeerIp.c_str(), uPeerPort, uPktLen);
...
void CDataHandler::__requestDispatch(Request& request, evwork::IConn* pConn)
{
	...
	{
		LOG(Error, "[ds::CDataHandler::%s] catch exeception : %s", __FUNCTION__, e.what());
	}
}
