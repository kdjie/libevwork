...
{
	LOG(Notice, "[ds::CThreadWorker::%s] tid:%u running...", __FUNCTION__, syscall(__NR_gettid));
	...
	LOG(Notice, "[ds::CThreadWorker::%s] tid:%u exit!", __FUNCTION__, syscall(__NR_gettid));
}
...
void CAsyncDataHandler::__requestDispatch(Request& request, evwork::IConn* pConn)
{
	...
	{
		LOG(Error, "[ds::CAsyncDataHandler::%s] catch exeception : %s", __FUNCTION__, e.what());
	}
}

bool CAsyncDataHandler::__cbTimerPrint()
{
	LOG(Notice, "[ds::CDataHandler::%s] request:%u proc:%u bytes:%llu", __FUNCTION__, m_requestQueue.size(), m_uProc, m_uBytes64);
